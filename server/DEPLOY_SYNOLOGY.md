...
1. 打开 **File Station**，进入共享文件夹（如 `docker` 或 `data`），新建文件夹 `tapledger`。
...
然后在**同一个 `tapledger` 文件夹**里再建一个 `.env` 文件（不要提交到任何 git），内容：
```
JWT_SECRET=这里填你的随机长字符串（至少32位）
```
> ⚠️ **不要**把 `.env` 放到任何仓库/网盘里，它包含你的服务端签名密钥，泄露会导致账号伪造。
...
3. 填：
   - **名称**：`tapledger`
   - **路径**：找到刚才的 `tapledger` 文件夹（含 `docker-compose.yml` 的那一层）
...
出现类似输出即成功：
```json
{"status":"ok","version":"0.4.1","time":1720000000000}
```
...
连接后即可多人共享同一个家庭账本，数据全部存在你的 NAS（`tapledger/data/`）。
...
| 想换新版本 | 镜像由 **GitHub Actions 自动发布**（服务端代码一合并到 main 就自动构建 + 推 latest）。你只需进项目 → **更新**：重新拉取 latest 再「重新创建」即可，或手动在上面的 yml 里指定 `image: ...:具体版本号` 更稳定 |
| 数据会不会丢 | 不会。数据库在 `tapledger/data/tapledger.db`，**定期备份这个文件夹即可** |

---

## 7. 升级 / 备份要点

- **升级**：改 yml 里 `image: ghcr.io/sctale/tapledger-server:新版本号` → 项目里重新创建容器即可，数据保留在 volume。
- **备份**：停掉容器后整包复制 `tapledger/data/`，或用 DSM 的「Hyper Backup」备份该文件夹。
