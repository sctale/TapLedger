...
export const RecordRow = React.memo(function RecordRow({
  record, onDelete, accountNames, showTime,
}: {
  record: LedgerRecord;
  onDelete?: (record: LedgerRecord) => void;
...
// 记录列表（暖色卡片风格）
function RecordList({ records, onDelete, emptyText = '还没有记录，记一笔吧 ✨', accountNames, showTime }: Props) {
  if (records.length === 0) {
    return (
  ...
  return (
    <View style={styles.list}>
      {records.map((record) => (
        <RecordRow key={record.id} record={record} onDelete={onDelete} accountNames={accountNames} showTime={showTime} />
      ))}
    </View>
  );
}
